...
const blogConfig = steadConfig.blog;
...
export function getBlogPathWithBase(path: string, ignoreTrailingSlash = false) {
  ...
  return getPathWithBase(
    path ? `/${blogConfig.prefix}/${path}` : `/${blogConfig.prefix}`,
    ignoreTrailingSlash,
  );
}
...
export function getPathWithBase(path: string, ignoreTrailingSlash = false) {
  ...
  const trailingSlashTransformer =
    trailingSlashTransformers[steadConfig.astro.trailingSlash ?? "ignore"];

  return trailingSlashTransformer(path);
}

export function isAnyBlogPage(slug: string) {
  return slug.match(new RegExp(`^${blogConfig.prefix}(/?$|/.+/?$)`)) !== null;
}

export function isBlogRoot(slug: string) {
  return slug === blogConfig.prefix;
}

export function isAnyBlogPostPage(slug: string) {
  return (
    slug.match(new RegExp(`^${blogConfig.prefix}/(?!(\\d+/?|tags/.+)$).+$`)) !==
    null
  );
}
...
export function isBlogTagsPage(slug: string, tag: string) {
  return slug === `${blogConfig.prefix}/tags/${tag}`;
}
