...
import type { StarlightBlogAuthor, blogSchema } from "@content/blog";
...
export async function getBlogStaticPaths() {
  ...
  for (const entry of entries) {
    ...
    if (!lastPage || lastPage.length === steadConfig.blog.postCount) {
  ...
  return entryPages.map((entries, index) => {
    ...
    const prevLink = prevPage
      ? {
          href: getBlogPathWithBase(index === 1 ? "/" : `/${index}`),
          label: "Newer posts",
        }
      : undefined;
    ...
    const nextLink = nextPage
      ? { href: getBlogPathWithBase(`/${index + 2}`), label: "Older posts" }
      : undefined;

    return {
      params: {
        page: index === 0 ? undefined : index + 1,
        prefix: steadConfig.blog.prefix,
      },
      props: {
        entries,
        nextLink:
          steadConfig.blog.prevNextLinksOrder === "reverse-chronological"
            ? nextLink
            : prevLink,
        prevLink:
          steadConfig.blog.prevNextLinksOrder === "reverse-chronological"
...
export async function getSidebarBlogEntries() {
  ...
  return {
    featured,
    recent: recent.slice(0, steadConfig.blog.recentPostCount),
  };
}
...
export async function getBlogEntry(
  ...
  const entryIndex = entries.findIndex(
    (entry) => entry.slug === slug.replace(/^\//, "").replace(/\/$/, ""),
  );
  ...
  const prevLink = prevEntry
    ? {
        href: getPathWithBase(`/${prevEntry.slug}`),
  ...
  const nextLink = nextEntry
    ? {
        href: getPathWithBase(`/${nextEntry.slug}`),
...
export function getBlogEntryMetadata(
  entry: StarlightBlogEntry,
): StarlightBlogEntryMetadata {
  const authors: StarlightBlogAuthor[] = [];

  if (!entry.data.authors) {
    authors.push(...Object.values(steadConfig.blog.authors));
  ...
  return {
    authors,
    date: entry.data.date.toLocaleDateString(
      steadConfig.starlight.defaultLocale,
      {
        dateStyle: "medium",
      },
    ),
    lastUpdated:
      typeof entry.data.lastUpdated === "boolean"
        ? undefined
        : entry.data.lastUpdated?.toLocaleDateString(
            steadConfig.starlight.defaultLocale,
            {
              dateStyle: "medium",
            },
...
export async function getBlogEntries() {
  const entries = await getCollection("docs", ({ id, data }) => {
    return (
      id.startsWith(`${steadConfig.blog.prefix}/`) &&
      id !== `${steadConfig.blog.prefix}/index.mdx` &&
...
// them for the docs.
function validateBlogEntries(
  entries: StarlightEntry[],
): asserts entries is StarlightBlogEntry[] {
  for (const entry of entries) {
    validateBlogEntry(entry);
  }
}
...
): asserts entry is StarlightBlogEntry {
...
function getAuthorFromConfig(id: string): StarlightBlogAuthor {
  const author = steadConfig.blog.authors[id];
...
// type StarlightEntryData = z.infer<ReturnType<typeof blogSchema>> & {
//   draft?: boolean;
//   title: string;
// };
